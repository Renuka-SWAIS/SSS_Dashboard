...
export default function AiTranslatorPage() {
  ...
  async function handleTranslate() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.detail || "Unable to translate text."
        );
      }
      ...
      setError(
        translateError.message || "Unable to translate text."
      );
      setStatus("");
    } finally {
      setLoading(false);
  ...
  async function handleCopy() {
    ...
    try {
      ...
      setError("Unable to copy translated text.");
    }
  }
  ...
  function handleSpeak() {
    ...
    const speechLanguage =
      speechLanguages[targetLanguage] || "en-IN";
    ...
    if (!matchingVoice && voices.length > 0) {
      setError(
        `${targetLanguage} voice is not available in this browser/device.`
      );
      setStatus("");
      return;
    }
    ...
    speech.onerror = () => {
      setError(
        `Unable to speak the translated text in ${targetLanguage}.`
      );
      setStatus("");
    };
      ...
      <section className="module-page">
        ...
        <div className="module-content-area">
          <article className="module-card translator-card">
            <div className="card-title-row">
              <div>
                <h2>AI Translator</h2>
                <p>
                  Translate study text into another language script.
                </p>
              </div>
          ...
          <div className="note-box">
            Use AI Translator for study notes, chapter paragraphs,
